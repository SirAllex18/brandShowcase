  ...
  try {
      ...
      const chatHistory = [
        {
          role: "system",
          content:
            "You are a helpful assistant specialized in football statistics and predictions. Provided only the win percentage back without any additional text.",
        },
        {
          role: "user",
    ...
    const result = await Promise.all(
      selectedFixtures.map(async (fixture, index) => {
        ...
        const saveGameInformation = new Game(gameData);
        const savedGame = await saveGameInformation.save()
      })
    );

    await statsDocument.save();
    console.log("Succesfull job");
  } catch (err) {
    console.log(err);
  }
};
